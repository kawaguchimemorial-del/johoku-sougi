...
**テスト送信**: 本番 /contact/ から1件送信（POST 200・Webhook エラーなし）。運営者がテストメールの受信を確認済み（2026-09-24）。フォームは本番で稼働中。
...
- 本番のリダイレクトを Googlebot UA で実測：http→https / apex→www / 末尾スラッシュ補完 いずれも308で1回ずつ、最長3ホップで正常着地。ループ・壊れたLocationなし。
- sitemap 全81URLは全て200。
- URL検査API（サービスアカウント）で81URL全件を判定：79件「登録済み」、2件「未認識」（`/hall/machiya-saijo/` `/column/juzu-erabi/` は新設でクロール待ち）。**「リダイレクト エラー」判定は0件**。

**結論**: 正規URL側に問題なし。対象は非www / http / スラッシュなし等の変種URL（6/24のwww統一以前に拾われたもの）か一時的なクロール失敗と推定。コード変更は行っていない。

**あなた側の作業**: SCの「リダイレクト エラー」の該当URL一覧を確認し、変種URLだけなら「修正を検証」を押す。想定外のURLがあればClaudeに共有（リダイレクト追加で対応）。
